...
const Goals = () => {
  ...
  const handleAddFunds = () => {
    ...
    setGoals((prevGoals) =>
      prevGoals.map((goal) => {
        if (goal.id === selectedGoal.id) {
          ...
          return { ...goal, saved: newSaved, percentage: newPercentage, remaining: newRemaining };
        }
        return goal;
      })
    );
    ...
    toast({
      title: "Funds Added",
      description: `Successfully added $${amountToAdd.toLocaleString()} to ${selectedGoal.name}`,
    });
    setAmount("");
  };
  ...
  const handleWithdraw = () => {
    ...
    toast({
      ...
      description: `Successfully withdrawn $${amountToWithdraw.toLocaleString()} from ${selectedGoal.name}`,
    });
    setAmount("");
  };
              ...
              return (
                ...
                >
                  <CardContent className="p-6 space-y-4">
                    ...
                    {/* Target Amount */}
                    <div>
                      <p className="text-3xl font-bold text-white">${goal.target.toLocaleString()}</p>
                    ...
                    <div>
                      <div className="flex justify-between mb-2">
                        ...
                        <span className="text-sm font-medium text-white">{goal.percentage}%</span>
                      </div>
                      <Progress
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between text-white/60">
                        <span>Remaining</span>
                        <span className="text-white/90">${goal.remaining.toLocaleString()}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {selectedGoal && (
            <>
              <DialogHeader>
                ...
                <p className="text-sm text-muted-foreground">Manage your goal progress and make contributions</p>
              </DialogHeader>
              ...
              <div className="space-y-6 mt-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-green-600 font-bold text-lg">{selectedGoal.percentage}%</span>
                  </div>
                  <Progress value={selectedGoal.percentage} className="h-3" />
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Saved</p>
                      <p className="text-green-600 font-semibold">${selectedGoal.saved.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Remaining</p>
                      <p className="font-semibold">${selectedGoal.remaining.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold">${selectedGoal.target.toLocaleString()}</p>
                    </div>
                    <div>
