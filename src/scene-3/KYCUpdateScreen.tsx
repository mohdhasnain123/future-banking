...
const KYCUpdateScreen = ({ onMainMenu, onExit }) => {
	...
	const [preparing, setPreparing] = useState(false); // Don't prepare on mount
	...
	useEffect(() => {
		if (!started) return;
		if (stepIndex === steps.length && !kycUpdating && !kycUpdateTick && !kycUpdated) {
	...
	// Handle transcript for confirmation
	useEffect(() => {
		if (stepIndex === steps.length && !kycUpdating && !kycUpdateTick && !kycUpdated) {
	...
	const handleConfirm = () => {
		...
		kycTimerRef.current = setTimeout(() => {
			...
			setKycUpdateTick(true); // Show tick after progress bar
	...
	useEffect(() => {
		if (stepIndex === steps.length && !kycUpdating && !kycUpdateTick && !kycUpdated && started) {
	...
	const handleScreenClick = (e) => {
		// Only allow toggling during confirmation step
		if (!(started && stepIndex === steps.length && !kycUpdating && !kycUpdateTick && !kycUpdated)) {
			return;
		}
		const y = e.clientY;
						...
						<>
							...
							{progressing && (
								<div style={{ margin: "18px 0" }}>
									...
									<div style={{ color: "#00eaff", marginTop: 8, fontSize: 16, letterSpacing: 1 }}>
										Recognizing...
									</div>
			...
			{started && stepIndex === steps.length && !kycUpdating && !kycUpdateTick && !kycUpdated && (
				...
				>
					...
					>
						...
						<div style={{ display: "flex", gap: 24, alignItems: "center", marginTop: "50px" }}>
							<button
